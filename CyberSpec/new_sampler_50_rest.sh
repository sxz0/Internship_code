...
server="https://deeplearning.inf.um.es"
port="5002"
directory="/sensor/"
mac=$( cat /sys/class/net/eth0/address | tr : _ )

#	Final and temporal output files
#finalOutput="/tmp/samples_$(date +'%Y-%m-%d-%H-%M')_${timeWindowSeconds}s"
...
#tempOutput=$(perf stat --log-fd 1 -e "$targetEvents" -a sleep 1)
...
#echo -n "time,timestamp,seconds,connectivity,${resourcePlaceholder}"

#placeholder=$(echo "$tempOutput" | cut -b 25- | cut -d " " -f 1 | tail -n +6 | head -n -3 | tr "\n" "," | sed 's/.$//')
...
do
	...
	#	PUSH to server
	finalOutput="$timeAcumulative,$timestamp,$seconds,$connectivity,${resourceSample}$sample"
